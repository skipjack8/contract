...
contract ShieldedTRC20 {
    ...
    function mint(uint256 rawValue, bytes32[9] calldata output, bytes32[2] calldata bindingSignature, bytes32[21] calldata c) external {
        ...
        leafCount ++;
        pushBlockNumber();// store current block number
    ...
    function setMaxNumberPerCall(uint256 _max) public {
        require(msg.sender == owner, "should be called by owner.");
        require(_max > 0, "_max should be positive.");
        maxNumberPerCall = _max;
    }

    function getBlockNumbers(uint256 from, uint256 to) public view returns(uint256[] memory) {
        require(from < to, "from must be smaller than to");
        require(from < blockNumbers.length && to <= blockNumbers.length, "from or to out of range");
        uint256 limit = to - from;
        require(limit <= maxNumberPerCall, "get to much elements at onces");

        uint256[] memory ret = new uint256[](limit);
        for (uint256 i = 0; i < limit; i++) {
            ret[i] = blockNumbers[i + from];
        }

        return ret;
    }

    function getBlockNumberLength() public view returns(uint256) {
    ...
    function pushBlockNumber() internal {
